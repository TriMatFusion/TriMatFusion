Welcome to our tutorial! This guide will walk you through the complete pipeline for training the tri-modal fusion model, from data collection to analyzing the final results.
...
* **Format:** The model uses **ASE (Atomic Simulation Environment)** to read these files. Any format supported by ASE is acceptable, including `.cif`, `POSCAR`, `ASE JSON`, etc.
...
The second modality, text descriptions, is generated by converting your crystal structures into natural language descriptions. This process is handled using **Robocrystallographer** (`robocrys`).
...
To analyze the model's interpretability (i.e., which words the model "focused on" in the text modality), we use the **Attention Visualizer** tool.
